...
class algo:
    ...
    def basic(self, limit):
        ...
        '''
        if self.last < int(self.high) * limit/100 and self.last <= self.tx_hr_price_avg and self.last <= self.tx_10min_price_avg and self.ask <= int(self.last + 3):
            return True
        else:
            return False

    def slump(self, p1, p2, p3, p4 ):
        ...
        if self.tx_1min_price_delta < -(self.high * p1/100) or \
         (self.tx_1min_price_delta > p2/100 and self.tx_10min_price_delta < -(self.high * p3/100) and self.tx_hr_price_delta < self.tx_10min_price_delta * p4):
            return True
        else:
            return False
