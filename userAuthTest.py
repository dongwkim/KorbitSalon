...
logging.basicConfig(format='%(asctime)s %(levelname)s %(name)s %(message)s',filename='Auth.trc',level=logging.DEBUG)
...
if __name__ == "__main__":
    ...
    #print order
    print "{} Buy: id# {} is {}".format(bidorder['currencyPair'],bidorder['orderId'] ,bidorder['status'])

    myask = {"currency_pair" : "xrp_krw", "type":"limit", "price": "710", "coin_amount":"10", "nonce": getNonce()}
    askorder = askOrder(myask,header)
    print "{} Sell: id# {} is {}".format(askorder['currencyPair'],askorder['orderId'] ,askorder['status'])
    ...
    for i in range(len(cancel)):
        print("{} Cancel: id# {} is {}".format(cancel[i]['currencyPair'],cancel[i]['orderId'] ,cancel[i]['status']))
